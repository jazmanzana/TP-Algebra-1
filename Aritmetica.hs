...
inversoMultiplicativo m n = fst tupla_s_t
                          where (g, tupla_s_t) = mcdExt m n
-- Primero Calcula el mcdExt de dos números 'a' y 'b',
-- Lo cual devuelve una tupla (g,s,t)
-- Luego a·s ≡ 1 (mod b)
-- Es decir (a·s) `mod` b = 1
-- Entonces habría que devolver s
...
{- Función sumaDivisores
 - Devuelve la suma de todos los divisores de un número dado
 -}
